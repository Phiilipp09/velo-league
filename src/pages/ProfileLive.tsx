...
function CareerCards({ snapshots, seasonYear, points, wins, kilometers, elevation, jerseys, seasonClosed }: { snapshots: SeasonCardSnapshot[]; seasonYear: number; points: number; wins: number; kilometers: number; elevation: number; jerseys: JerseyKey[]; seasonClosed: boolean }) {
  ...
  const liveCard: SeasonCardSnapshot = { id: 'live', group_id: '', user_id: '', created_at: new Date().toISOString(), season_year: seasonYear, rarity: currentRarity, total_points: points, wins, kilometers, elevation_m: elevation, titles: jerseys.map(key => jerseyTitle[key]) }
  ...
  return <><div className="career-layout"><div className="season-card-strip">{allCards.length ? allCards.map((card, index) => <button type="button" aria-label={`Saisonkarte ${card.season_year} öffnen`} className={`season-mini-card season-mini-${card.rarity}`} key={card.id} onClick={() => setSelectedCard(card)}><span>{card.id === 'live' && index === 0 ? 'LIVE · AKTUELLE SAISON' : 'ARCHIV · SAISONKARTE'}</span><strong>{card.season_year}</strong><b>{card.titles[0] || 'DEBUT SEASON'}</b><small>{format(card.total_points)} PKT</small><div><span>{card.wins} Siege</span><span>{format(card.kilometers)} km</span></div><em>Details ansehen</em></button>) : <Card className="special-card-empty"><Medal size={22}/><div><strong>Deine Saisonkarte wird vorbereitet.</strong><p>Nach dem Saisonabschluss erscheint sie hier dauerhaft.</p></div></Card>}</div><Card className="career-growth"><div><p className="eyebrow">KARRIERE-ENTWICKLUNG</p><h3>Deine Geschichte, Saison für Saison.</h3></div><div className="career-chart career-timeline" aria-label="Entwicklung deiner Saisonpunkte">{careerSource.map(card => <button type="button" key={card.id} className={card.id === timelineCard?.id ? 'active' : ''} onClick={() => setSelectedTimelineId(card.id)} aria-label={`Saison ${card.season_year}: ${format(card.total_points)} Punkte`}><i style={{ height: `${Math.max(12, Math.round((card.total_points / highest) * 100))}%` }}/><span>{card.season_year}</span></button>)}</div>{timelineCard && <button type="button" className="career-highlight" onClick={() => setSelectedCard(timelineCard)}><span>SAISON {timelineCard.season_year}</span><strong>{format(timelineCard.total_points)} Punkte</strong><small>{timelineCard.wins} Siege · {format(timelineCard.kilometers)} km · Details öffnen</small></button>}<div className="career-summary"><span>{allCards.length} Saison{allCards.length === 1 ? '' : 'en'}</span><span>{format(careerKilometers)} Karriere-km</span><span>{careerTitles} Titel gewonnen</span><span>Bestes Jahr: {bestSeason.season_year} · {format(bestSeason.total_points)} Pkt</span></div></Card></div>{selectedCard && <SeasonCardModal card={selectedCard} onClose={() => setSelectedCard(null)}/>}</>
}
...
function PerformancePanel({ ratings, overall, stats, bike, onSettings }: { ratings: Rating[]; overall: number; stats: SeasonStats; bike: RiderProfile; onSettings: () => void }) { return <div className="performance-panel"><Card className="performance-rating"><div className="overall-rating"><span>OVR</span><strong>{overall}</strong><small>{stats.rides.length ? 'aus deinen Saisonwerten' : 'Rookie-Startwert'}</small></div><div className="rating-bars">{ratings.map(rating => <div key={rating.label}><span>{rating.label}<small>{rating.detail}</small></span><b><i style={{ width: `${rating.value}%` }}/></b><strong>{rating.value}</strong></div>)}</div></Card><Card className="performance-top"><p className="eyebrow">TOP-PERFORMANCES</p><div><span>Längste Ausfahrt</span><strong>{format(stats.longestKilometers)} km</strong></div><div><span>Höchste HM</span><strong>{format(stats.elevation)} hm</strong></div><div><span>Gesamtzeit</span><strong>{Math.round(stats.movingSeconds / 3600)} h</strong></div></Card><Card className="bike-card"><Bike size={26}/><div><p className="eyebrow">DEIN BIKE</p><h3>{bike.bikeBrand || bike.bikeModel ? [bike.bikeBrand, bike.bikeModel].filter(Boolean).join(' ') : 'Bike hinzufügen'}</h3><p>{bike.bikeType || 'Für deine Fahrerkarte und Leistungsübersicht.'}</p></div><button className="secondary-button" onClick={onSettings}>Bearbeiten</button></Card><Card className="rating-explainer"><p className="eyebrow">SO ENTSTEHT DEIN RATING</p><p>Die Bewertung nutzt ausschließlich echte Gruppenfahrten: Höhenmeter für Berg, Kilometer und längste Ausfahrt für Ausdauer, Fahrten für Aktivität sowie Punkte, Trikots und Duellsiege für Wettbewerb.</p></Card></div> }
function makeRatings(stats: SeasonStats, points: number, wins: number, jerseys: number): Rating[] { const clamp = (value: number) => Math.max(0, Math.min(99, Math.round(value))); const rides = stats.rides.length; return [{ label: 'Berg', value: clamp(stats.elevation / 90 + (stats.kilometers ? stats.elevation / stats.kilometers : 0) * 2), detail: `${format(stats.elevation)} hm` }, { label: 'Ausdauer', value: clamp(stats.kilometers / 9 + stats.longestKilometers / 3 + stats.movingSeconds / 3600 * 2), detail: `${format(stats.kilometers)} km · ${format(stats.longestKilometers)} km längste Fahrt` }, { label: 'Aktivität', value: clamp(rides * 7), detail: `${rides} Fahrten` }, { label: 'Wettbewerb', value: clamp(points / 14 + wins * 10 + jerseys * 8), detail: `${format(points)} Pkt · ${wins} Duellsiege` }] }
