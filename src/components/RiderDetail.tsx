...
export function RiderDetail({ rider, onClose }: { rider: LeagueRider; onClose: () => void }) {
  ...
  const ratings = [
    { label: 'Berg', value: Math.min(99, Math.round(rider.elevation / 90 + (rider.kilometers ? rider.elevation / rider.kilometers : 0) * 2)) },
    { label: 'Ausdauer', value: Math.min(99, Math.round(rider.kilometers / 9 + rider.longestKilometers / 3 + rider.movingSeconds / 3600 * 2)) },
    { label: 'Aktivität', value: Math.min(99, rider.ridesCount * 7) },
    { label: 'Wettbewerb', value: Math.min(99, Math.round(rider.points / 14)) },
  ]

  return <div className="modal-backdrop" onClick={onClose}><aside className={`public-rider-card public-rider-${jersey}`} onClick={event => event.stopPropagation()}><button className="drawer-close" aria-label="Fahrerkarte schließen" onClick={onClose}><X size={20}/></button><header><span>VELO <b>LEAGUE</b></span><small>FAHRERKARTE</small></header><div className="public-rider-hero"><div><p>SAISONAKTUELL</p><div className="public-rider-number-row"><strong>#{String(rider.riderNumber || 0).padStart(2, '0')}</strong><div className="public-rider-overall public-rider-overall-diamond"><span>OVR</span><b>{rider.overall}</b></div></div><h2>{rider.name}</h2><span>{rider.team || 'INDEPENDENT RIDERS'}</span><i>{rarity} · RIDE FURTHER</i></div><div className="public-rider-image"><img src={riderImage} alt={`Rennradfahrer im ${jersey}-Trikot`}/></div></div><div className="public-rider-stats"><span><Trophy size={15}/><b>{format(rider.points)}</b><small>PKT</small></span><span><Mountain size={15}/><b>{format(rider.elevation)}</b><small>HM</small></span><span><Route size={15}/><b>{format(rider.kilometers)}</b><small>KM</small></span><span><Zap size={15}/><b>{format(rider.week)}</b><small>7 TAGE</small></span></div><div className="public-rider-ratings"><p>FAHRER-RATING</p>{ratings.map(item => <div key={item.label}><span>{item.label}</span><i><b style={{ width: `${item.value}%` }}/></i><strong>{item.value}</strong></div>)}</div><div className="public-rider-titles"><p>TITEL & STATUS</p><div><Jersey type={jersey} small/><span><strong>{jersey === 'plain' ? 'ERSTE WERTUNG WARTET' : 'AKTUELLES TRIKOT'}</strong><small>{jersey === 'plain' ? 'Sammle Punkte für deine erste Spezialkarte.' : 'In dieser Ranglistenwertung führend.'}</small></span></div></div><footer><p>Öffentliche Saisonkarte · nur für Mitglieder dieser Liga sichtbar.</p></footer></aside></div>
}
