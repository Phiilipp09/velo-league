...
export default function App() {
  ...
  useEffect(() => { void restoreSupabaseUser().then(async remoteUser => { if (remoteUser) { const name = getStoredSession() || remoteUser.user_metadata?.display_name || remoteUser.email?.split('@')[0] || 'Rider'; localStorage.removeItem('velo-demo-mode'); setDemoMode(false); setNotes([]); setUserId(remoteUser.id); setUser(name); const profile = await getProfile(remoteUser.id).catch(() => null); setProfileSetup(profile ? !profile.onboarding_completed : !localStorage.getItem(`velo-profile-complete:${name}`)) } }) }, [])
