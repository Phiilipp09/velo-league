...
export default function App() {
  ...
  return <div className="app-shell"><Sidebar page={page} setPage={setPage}/><main className="main-content"><header className="topbar"><div><p className="eyebrow">SAISON 2026 · ALPINE DIVISION</p><h1>{pages[page].label}</h1></div><div className="top-actions"><button className="icon-button notification-button" onClick={() => setNotifications(!notifications)} aria-label="Benachrichtigungen"><Bell size={19}/>{notes.length > 0 && <i/>}</button><button className="icon-button" onClick={() => setPage('league')} aria-label="Leistungsübersicht"><BarChart3 size={19}/></button><button className="avatar" onClick={() => setPage('profile')} aria-label="Profil">{user.slice(0, 1).toUpperCase()}</button></div></header>{page === 'dashboard' && <Dashboard empty={emptyStart} onNavigate={setPage}/>} {page === 'group' && <GroupHub notify={showNotice} onNavigate={setPage}/>} {page === 'league' && <League empty={emptyStart} onNavigate={setPage} onRiderSelect={setRider}/>} {page === 'activities' && <ActivityHub notify={showNotice}/>} {page === 'calendar' && <SeasonCalendar notify={showNotice}/>} {page === 'segments' && <Segments empty={emptyStart} notify={showNotice}/>} {page === 'challenges' && <Challenges empty={emptyStart} notify={showNotice}/>} {page === 'profile' && <Profile/>}</main>{notice && <div className="toast">{notice}</div>}{rider && <RiderDetail name={rider} onClose={() => setRider(null)}/>} {notifications && <Notifications notes={notes} onDelete={deleteNote} onClose={() => setNotifications(false)} onNavigate={setPage}/>} {onboarding && <Onboarding onSkip={() => setOnboarding(false)} onFinish={() => { setOnboarding(false); setPage('group'); showNotice('Willkommen in deiner ersten Liga!') }}/>}</div>
}
