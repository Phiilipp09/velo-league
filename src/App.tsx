...
import { League, type LeagueRider } from './pages/League'
...
import { deleteNotification, getNotifications, getProfile } from './lib/supabaseData'
...
export default function App() {
  ...
  const [rider, setRider] = useState<LeagueRider | null>(null)
  ...
  if (!user) return <AuthGate onAuthenticated={(name, id) => { const demo = localStorage.getItem('velo-demo-mode') === 'true'; setDemoMode(demo); setNotes(demo ? defaultNotes : []); setUserId(id); setUser(name); if (!id) { setProfileSetup(false); return } void getProfile(id).then(profile => { const configured = Boolean(profile?.onboarding_completed || profile?.gender || profile?.height_cm || profile?.weight_kg || profile?.rider_level); setProfileSetup(!configured) }).catch(() => setProfileSetup(!localStorage.getItem(`velo-profile-complete:${name}`))) }}/>
  ...
  return <div className="app-shell"><Sidebar demo={demoMode} page={page} setPage={setPage} stats={liveSeason.stats}/><main className="main-content"><header className="topbar"><div><p className="eyebrow">SAISON 2026 · ALPINE DIVISION</p><h1>{pages[page].label}</h1></div><div className="top-actions"><button className="icon-button notification-button" onClick={() => setNotifications(!notifications)} aria-label="Benachrichtigungen"><Bell size={19}/>{notes.length > 0 && <i/>}</button><button className="icon-button" onClick={() => setPerformance(true)} aria-label="Leistungsanalyse"><BarChart3 size={19}/></button><button className="avatar" onClick={() => setPage('profile')} aria-label="Profil">{user.slice(0, 1).toUpperCase()}</button></div></header>{page === 'dashboard' && <Dashboard empty={emptyStart} stats={liveSeason.stats} user={user} userId={userId} groups={liveSeason.groups} hasGroup={hasGroup} onNavigate={setPage}/>} {page === 'group' && <GroupHub userId={userId} userName={user} notify={showNotice} onNavigate={setPage}/>} {page === 'league' && <League demo={demoMode} userId={userId} groups={liveSeason.groups} onNavigate={setPage} onRiderSelect={setRider}/>} {page === 'activities' && <ActivityHub userId={userId} notify={showNotice}/>} {page === 'calendar' && <SeasonCalendar notify={showNotice} onNavigate={setPage}/>} {page === 'segments' && <Segments empty={!demoMode && liveSeason.stats.rides.length === 0} notify={showNotice}/>} {page === 'challenges' && <Challenges userId={userId} groups={liveSeason.groups} onNavigate={setPage} notify={showNotice}/>} {page === 'profile' && (demoMode ? <Profile/> : <ProfileLive user={user} userId={userId} groups={liveSeason.groups} stats={liveSeason.stats} hasGroup={hasGroup}/>)}</main>{notice && <div className="toast">{notice}</div>}{rider && <RiderDetail rider={rider} onClose={() => setRider(null)}/>} {performance && <PerformanceInsights demo={demoMode} onClose={() => setPerformance(false)}/>} {notifications && <Notifications notes={notes} onDelete={deleteNote} onClose={() => setNotifications(false)} onNavigate={setPage}/>} {onboarding && <Onboarding onSkip={() => setOnboarding(false)} onFinish={() => { setOnboarding(false); setPage('group'); showNotice('Willkommen in deiner ersten Liga!') }}/>}</div>
}
